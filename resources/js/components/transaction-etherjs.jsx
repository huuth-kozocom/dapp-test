...
export default function TransactionEtherjs() {
    ...
    const getTransactions = () => {
      axios.get(`http://localhost:8001/api/transaction-etherjs`).then(({data})=>{
          setTransactions(data);
      })
    }

    const createTransactions = (data) => {
      axios.post(`http://localhost:8001/api/transaction-etherjs/create`, data)
      .then((res) => {
        if (res) {
    ...
    const getGasPrice = async () => {
      try {
        ...
        const gasPrice = Number(ethers.utils.formatUnits(res.data.result.toString(), 'gwei')).toFixed(1);
    ...
    const getBalance = (address) => {
      ...
      .then (
        balance => setWalletBalance(ethers.utils.formatEther(balance))
      ).catch ((err) => {
        console.log(err);
      })
    }
    ...
    const sendTransaction = async () => {
      ...
      try {
        const tx = {
          from: walletAddress,
          to: masterWallet,
          gasLimit: ethers.utils.formatUnits("210000", "wei"),
    ...
    return (
      <div className="container">
          <h2 style={{textAlign:"center"}}> Transaction token etherjs demo </h2>
          <div className="row">
